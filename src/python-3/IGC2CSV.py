"""
This script matches the original written by OverloadUT, but it has been converted to Python 3 by Nicolas Huber using the 2to3 tool.
"""
...
def crunch_flight(flight):
    for index, record in enumerate(flight["fixrecords"]):
        ...
        if index > 0:
            ...
            record["distance_delta"] = haversine(
                record["londegrees"],
                record["latdegrees"],
            ...
            record["distance_from_start"] = straight_line_distance(
                record["londegrees"],
                record["latdegrees"],
                record["alt-GPS"],
...
def straight_line_distance(lon1, lat1, alt1, lon2, lat2, alt2):
    ...
    b = (alt1 - alt2) / 1000.0  # altitude is in meters, but we're working in km here
    c = sqrt(a**2.0 + b**2.0)
    return c
